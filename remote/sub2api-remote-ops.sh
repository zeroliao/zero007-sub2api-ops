...
sync_sidecar_proxies() {
  ...
  log "Permanently removing sidecar endpoints whose proxy records were deleted."
  compose exec -T postgres psql -U "${POSTGRES_USER:-sub2api}" -d "${POSTGRES_DB:-sub2api}" -v ON_ERROR_STOP=1 <<'SQL'
DELETE FROM proxy_sidecar_endpoints e
WHERE e.proxy_id IS NULL
   OR EXISTS (
     SELECT 1 FROM proxies p
     WHERE p.id = e.proxy_id AND p.deleted_at IS NOT NULL
   );
...
USING proxy_sidecar_endpoints e
JOIN proxy_subscription_nodes n ON n.id = e.node_id
JOIN proxy_subscription_sources s ON s.id = n.source_id
...
DELETE FROM proxy_sidecar_endpoints e
WHERE e.node_id = ANY(COALESCE(string_to_array(NULLIF(:'failed_nodes', ''), ',')::bigint[], ARRAY[]::bigint[]));

DELETE FROM proxies p
USING proxy_subscription_nodes n
JOIN proxy_subscription_sources s ON s.id = n.source_id
JOIN proxy_sidecar_endpoints e ON e.node_id = n.id
WHERE e.proxy_id = p.id
  AND COALESCE(p.source, 'manual') = 'subscription'
  AND (s.deleted_at IS NOT NULL OR s.status <> 'active' OR n.deleted_at IS NOT NULL OR n.selected <> TRUE
       OR e.node_id <> ALL(COALESCE(string_to_array(NULLIF(:'generated_nodes', ''), ',')::bigint[], ARRAY[]::bigint[])));
...
DELETE FROM proxy_sidecar_endpoints e
USING proxy_subscription_nodes n, proxy_subscription_sources s
WHERE e.node_id = n.id
  AND n.source_id = s.id
  AND (s.deleted_at IS NOT NULL OR s.status <> 'active' OR n.deleted_at IS NOT NULL OR n.selected <> TRUE
       OR e.node_id <> ALL(COALESCE(string_to_array(NULLIF(:'generated_nodes', ''), ',')::bigint[], ARRAY[]::bigint[])));
